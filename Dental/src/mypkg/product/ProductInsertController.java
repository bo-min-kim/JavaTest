...
public class ProductInsertController extends SuperClass{
	...
	public boolean validate(HttpServletRequest request) {
		...
		if( bean.getName().length() < 1 || bean.getName().length() > 15 ){
			request.setAttribute( super.PREFIX + "name", "상품 이름은 1자리 이상 15자리 이하이어야 합니다.");
			isCheck = false  ;
		}
		
		if( bean.getCompany().length() < 1 || bean.getCompany().length() > 30 ){
			request.setAttribute( super.PREFIX + "company", "제조 회사 이름은 1자리 이상 30자리 이하이어야 합니다.");
			isCheck = false  ;
		}
		...
		if( bean.getPoint() < 5 || bean.getPoint() > 10 ){
			request.setAttribute( super.PREFIX + "point", "포인트는 최소 1 이상 100 이하로 입력 하셔야 합니다.");
			isCheck = false  ;
		}
